...
export default function SwagClubLanding() {
  ...
  return (
    <div className={`min-h-screen relative transition-colors duration-500 font-sans selection:bg-teal-400 selection:text-black ${isDark ? 'text-white' : 'text-neutral-900'}`}>
      ...
      {/* Mobile Warning Component Injection */}
      <MobileWarning />
      ...
      <main className="relative pt-32 md:pt-40 pb-12 flex flex-col items-center overflow-hidden min-h-screen">
        ...
        <div className="relative z-10 text-center max-w-4xl mx-auto space-y-4 mb-12 px-4">
          ...
          <p className={`text-lg md:text-xl max-w-lg mx-auto mt-6 font-light ${isDark ? 'text-neutral-400' : 'text-neutral-500'}`}>
            The first cultural protocol onchain showcasing products through blockchain
          </p>
        </div>
        ...
        <div className="relative z-20 px-6">
          <button 
            ...
            className={`group flex items-center gap-3 px-8 py-4 rounded-full text-lg font-syne font-bold transition-all hover:scale-105 hover:shadow-lg ${
              isDark 
      ...
      {/* --- FORM MODAL (Keep the functionality!) --- */}
      {isApplyOpen && (
              ...
              {applyStep === 1 && (
                <div className="space-y-4 animate-in slide-in-from-right-8 duration-300">
                  ...
                  <div className="flex justify-end pt-4">
                    <button type="button" onClick={handleNextStep} disabled={!formData.inspiration} className="flex items-center gap-2 bg-teal-500 text-black px-6 py-3 rounded-full font-bold hover:bg-teal-400 disabled:opacity-50">Next <ChevronRight className="w-4 h-4" /></button>
                  </div>
                </div>
              )}
              {applyStep === 2 && (
                <div className="space-y-4 animate-in slide-in-from-right-8 duration-300">
                  ...
                  <textarea 
                    autoFocus
                    rows={4}
                    placeholder="Is it for fun? For the culture?"
                    value={formData.reason}
                    onChange={(e) => setFormData({...formData, reason: e.target.value})}
                    className={`w-full p-4 rounded-xl text-lg outline-none focus:ring-2 focus:ring-teal-500 transition-all ${isDark ? 'bg-white/5' : 'bg-neutral-100'}`}
                  />
                  <div className="flex justify-end pt-4">
                    <button type="button" onClick={handleNextStep} disabled={!formData.reason} className="flex items-center gap-2 bg-teal-500 text-black px-6 py-3 rounded-full font-bold hover:bg-teal-400 disabled:opacity-50">Next <ChevronRight className="w-4 h-4" /></button>
                  </div>
                </div>
              )}
              {applyStep === 3 && (
...
// Mobile Warning (Re-added here for safety)
