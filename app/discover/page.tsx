...
import { 
  ...
  Sun, Moon, ShoppingBag, X, Trophy, RefreshCw, Lock, CheckCircle2
} from 'lucide-react';
...
export default function DiscoverDashboard() {
  ...
  // Default to Dark Mode for the "Vogue" aesthetic
  ...
  useEffect(() => {
      ...
      try {
        ...
        for (let i = Number(totalIds); i > Math.max(0, Number(totalIds) - 10); i--) {
          try {
            ...
            // C. V3 CHECK: Did I enter this specific raffle?
            ...
            const raffleItem = {
                ...
                active: details[3],
                winner: details[4],
                hasEntered: hasEntered, // New V3 Property
    ...
    scanChain();
  }, [publicClient, address]); // Re-scan if wallet changes
            ...
            <>
              ...
              {isLoading && (
                 <div className="w-full h-[500px] flex items-center justify-center border border-dashed rounded-[40px] opacity-50">
                    <Loader2 className="animate-spin mr-2"/> Scanning Base Sepolia...
                 </div>
              )}
              ...
              {!isLoading && liveRaffles.length === 0 && (
                 <div className="w-full h-[400px] bg-neutral-100 dark:bg-neutral-900 rounded-[40px] flex flex-col items-center justify-center text-center p-8">
                    ...
                    <p className="text-neutral-500 max-w-md">The gallery is currently empty. Check back for the next cultural wave.</p>
                 </div>
              )}
                      ...
                      <div className="w-full md:w-[450px] bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl p-8 flex flex-col justify-between hover:bg-white/15 transition-all">
                         <div>
                            <div className="flex items-center gap-2 mb-6">
                               <span className="px-3 py-1 rounded-full bg-white text-black text-xs font-bold uppercase tracking-wider">Verified Drop</span>
                            ...
                            <div className="mb-6">
                                <div className="flex justify-between text-xs text-neutral-300 mb-2">
                                    <span>{activeRaffle.sold} / {activeRaffle.max} Tickets</span>
                                    <span>{activeRaffle.progress.toFixed(0)}% Sold</span>
                                </div>
                                <div className="w-full h-3 bg-white/10 rounded-full overflow-hidden border border-white/5">
                                    <div className="h-full bg-gradient-to-r from-teal-400 to-purple-500 transition-all duration-700" style={{ width: `${activeRaffle.progress}%` }} />
                                </div>
                            </div>

                            <div className="space-y-3 text-sm text-neutral-200">
                               <div className="flex items-center gap-3"><MapPin className="w-4 h-4 opacity-50"/> Base Sepolia Network</div>
                            </div>
                         </div>
                         
                         <div className="mt-6 pt-6 border-t border-white/10">
                            <p className="text-xs text-neutral-400 uppercase tracking-widest mb-2">Entry Price</p>
                      ...
                      <div className="flex-1 flex flex-col justify-end items-end">
                         ...
                         <div className="flex gap-2 mb-4">
                            {liveRaffles.length > 1 && (
                              <>
                                <button onClick={() => setCurrentSlide((c) => (c - 1 + liveRaffles.length) % liveRaffles.length)} className="p-3 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-md transition border border-white/10"><ChevronLeft className="w-5 h-5"/></button>
                                <button onClick={() => setCurrentSlide((c) => (c + 1) % liveRaffles.length)} className="p-3 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-md transition border border-white/10"><ChevronRight className="w-5 h-5"/></button>
                              </>
                            )}
                         </div>

                         {/* SMART BUTTON (V3 Logic) */}
                         <button 
                            onClick={() => !activeRaffle.hasEntered && setTicketModalOpen(true)} 
                            disabled={activeRaffle.hasEntered || activeRaffle.sold >= activeRaffle.max}
                            className={`group relative px-10 py-5 rounded-full font-bold text-lg transition-all shadow-[0_0_30px_rgba(255,255,255,0.2)] flex items-center gap-3 disabled:opacity-90 disabled:cursor-not-allowed
                                ${activeRaffle.hasEntered 
                                    ? 'bg-green-500 text-black border-2 border-green-400' 
                                    : activeRaffle.sold >= activeRaffle.max 
                                        ? 'bg-neutral-800 text-neutral-400' 
                                        : 'bg-white text-black hover:bg-teal-400 hover:border-teal-400'}
                            `}
                         >
                            {activeRaffle.hasEntered ? (
                                <>Entry Confirmed <CheckCircle2 className="w-6 h-6"/></>
                            ) : activeRaffle.sold >= activeRaffle.max ? (
                                <>Sold Out <Lock className="w-5 h-5"/></>
                            ) : (
                                <>Mint Ticket <ChevronRight className="w-5 h-5 group-hover:translate-x-1 transition-transform"/></>
                            )}
                         </button>

                         {activeRaffle.hasEntered && (
                             <p className="text-green-400 text-sm mt-3 font-bold text-shadow">You are in the draw.</p>
                         )}
                      </div>
                   </div>
                </section>
             ...
             <div className="animate-in fade-in slide-in-from-bottom-4">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
                   <div>
                      ...
                      <p className="text-neutral-500 mt-2">Instant liquidity for your positions.</p>
                   </div>
                </div>
                <div className={`p-12 border border-dashed rounded-[40px] text-center text-neutral-500 ${isDark ? 'border-white/10' : 'border-black/10'}`}>
                    ...
                    <h3 className="text-lg font-bold mb-2">P2P Trading Coming Soon</h3>
                    <p className="text-sm max-w-md mx-auto">V2 Marketplace contracts are being audited. In the meantime, use the Wallet tab to instantly swap active tickets for 70% liquidity.</p>
                </div>
             </div>
          )}
                           ...
                           <div className="mt-auto pt-4 border-t border-dashed border-white/10">
                               {ticket.status === 'ACTIVE' ? (
                                   <button 
                                     ...
                                     className="w-full flex items-center justify-center gap-2 text-sm font-bold bg-yellow-500/10 text-yellow-500 px-4 py-3 rounded-xl hover:bg-yellow-500 hover:text-black transition-all"
                                   >
                                     <RefreshCw className="w-4 h-4"/> Swap for 70% Liquidity
                                   </button>
                               ) : ticket.status === 'WON' ? (
                                   <div className="w-full py-3 text-center text-green-400 font-bold bg-green-500/10 rounded-xl flex items-center justify-center gap-2">
                                       <Trophy className="w-4 h-4"/> Prize in Wallet
                                   </div>
                               ) : (
                                   <div className="w-full py-3 text-center text-neutral-500 font-bold bg-white/5 rounded-xl">
      ...
      {ticketModalOpen && activeRaffle && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/60 backdrop-blur-md animate-in fade-in">
           <div className="bg-white w-full max-w-md p-8 rounded-[40px] relative shadow-2xl text-black">
              ...
              <div className="bg-neutral-50 p-6 rounded-3xl mb-8 border border-neutral-200 text-center">
                 <p className="text-xs text-neutral-500 uppercase tracking-widest mb-1">Total Cost</p>
                 <span className="text-4xl font-bold font-grotesk text-teal-600">{activeRaffle.price} ETH</span>
              </div>

              <button 
                 onClick={handleBuyTicket} 
                 disabled={isPending || isSuccess} 
                 className="w-full py-5 bg-black text-white rounded-full font-bold text-xl hover:bg-neutral-800 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
              >
                 {isPending ? <><Loader2 className="animate-spin"/> Confirming...</> : isSuccess ? <><CheckCircle2/> Success!</> : 'Confirm Payment'}
              </button>
