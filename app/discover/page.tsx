...
import { 
  ...
  Sun, Moon, ShoppingBag, X, Trophy, RefreshCw
} from 'lucide-react';
        ...
        <div className="px-6 md:px-8 pb-32 max-w-[1600px] mx-auto">
          ...
          {activeTab === 'Home' && (
            <>
              ...
              {isLoading && <div className="w-full h-[500px] flex items-center justify-center border border-dashed rounded-[40px] opacity-50"><Loader2 className="animate-spin mr-2"/> Syncing with Base...</div>}
              {!isLoading && liveRaffles.length === 0 && <div className="w-full h-[400px] bg-neutral-100 dark:bg-neutral-900 rounded-[40px] flex flex-col items-center justify-center text-center p-8"><Ticket className="w-16 h-16 text-neutral-300 mb-4"/><h3 className="text-xl font-bold mb-2">No Active Drops</h3><p className="text-neutral-500 max-w-md">There are no active raffles right now. Check back later.</p></div>}
                ...
                <section className="relative w-full min-h-[500px] md:h-[600px] rounded-[40px] overflow-hidden bg-black text-white shadow-2xl group flex flex-col md:block">
                   ...
                   <div className="relative h-full p-8 md:p-12 flex flex-col md:flex-row gap-8 z-10">
                      <div className="w-full md:w-[450px] bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl p-8 flex flex-col justify-between hover:bg-white/15 transition-all">
                         <div>
                            <div className="flex items-center gap-2 mb-6"><span className="px-3 py-1 rounded-full bg-white text-black text-xs font-bold uppercase tracking-wider">Live Raffle</span><span className="w-2 h-2 rounded-full bg-green-400 animate-pulse"/></div>
                            ...
                            <div className="mb-6">
                                <div className="flex justify-between text-xs text-neutral-300 mb-2"><span>{activeRaffle.sold} / {activeRaffle.max} Tickets Sold</span><span>{activeRaffle.progress.toFixed(0)}%</span></div>
                                <div className="w-full h-3 bg-white/10 rounded-full overflow-hidden"><div className="h-full bg-teal-400 transition-all duration-500" style={{ width: `${activeRaffle.progress}%` }} /></div>
                            </div>
                            <div className="space-y-3 text-sm text-neutral-200"><div className="flex items-center gap-3"><MapPin className="w-4 h-4 opacity-50"/> Base Sepolia</div></div>
                         </div>
                         <div className="mt-6"><p className="text-xs text-neutral-400 uppercase tracking-widest mb-2">Ticket Price</p><div className="text-4xl font-bold font-grotesk text-white mb-2">{activeRaffle.price} <span className="text-lg font-normal text-neutral-400">ETH</span></div></div>
                      </div>
                      <div className="flex-1 flex flex-col justify-end items-end">
                         <div className="flex gap-2 mb-4">
                            {liveRaffles.length > 1 && (<><button onClick={() => setCurrentSlide((c) => (c - 1 + liveRaffles.length) % liveRaffles.length)} className="p-3 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-md transition"><ChevronLeft className="w-5 h-5"/></button><button onClick={() => setCurrentSlide((c) => (c + 1) % liveRaffles.length)} className="p-3 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-md transition"><ChevronRight className="w-5 h-5"/></button></>)}
                         </div>
                         <button onClick={() => setTicketModalOpen(true)} disabled={activeRaffle.sold >= activeRaffle.max} className="group relative px-10 py-5 bg-white text-black rounded-full font-bold text-lg hover:bg-teal-400 transition-all shadow-[0_0_30px_rgba(255,255,255,0.3)] flex items-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed">
                            {activeRaffle.sold >= activeRaffle.max ? "Sold Out" : "Buy Ticket"} <ChevronRight className="w-5 h-5 group-hover:translate-x-1 transition-transform"/>
                         </button>
                      </div>
                   </div>
                </section>
          ...
          {activeTab === 'Market' && (
             <div className="animate-in fade-in">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4"><div><h2 className={`text-3xl font-bold font-grotesk ${isDark ? 'text-white' : 'text-black'}`}>Secondary Market</h2><p className="text-neutral-500 mt-2">Trade tickets or swap instantly for liquidity.</p></div></div>
                <div className="p-12 border border-dashed rounded-3xl text-center text-neutral-500"><p>P2P Marketplace listings coming in V2 Contract.</p><p className="text-sm mt-2">Use the <b>Wallet</b> tab to swap your active tickets for 70% instant liquidity.</p></div>
             </div>
          )}
          ...
          {activeTab === 'Wallet' && (
            <div className="animate-in slide-in-from-bottom-4">
               ...
               <div className={`p-8 rounded-[40px] mb-12 relative overflow-hidden flex flex-col md:flex-row justify-between items-center gap-8 ${isDark ? 'bg-neutral-900 border border-white/10' : 'bg-black text-white'}`}><div className="text-center md:text-left w-full"><p className="text-neutral-400 text-sm mb-2 uppercase tracking-widest">Available Balance</p><h3 className="text-4xl md:text-5xl font-grotesk font-bold text-teal-400">{ethBalance ? Number(ethBalance.formatted).toFixed(3) : '0.00'} <span className="text-lg text-white">ETH</span></h3></div></div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                     {myTickets.map((ticket, idx) => (
                        <div key={idx} className={`p-4 rounded-2xl border flex items-center justify-between ${isDark ? 'bg-white/5 border-white/10' : 'bg-white border-black/5'}`}>
                           <div className="flex items-center gap-4"><div className="w-16 h-16 rounded-xl overflow-hidden bg-gray-200"><img src={ticket.image} className="w-full h-full object-cover"/></div><div><h4 className={`font-bold ${isDark?'text-white':'text-black'}`}>{ticket.title}</h4><div className="flex items-center gap-2 mt-1">{ticket.status === 'WON' && <span className="text-xs font-bold text-green-500 bg-green-500/10 px-2 py-0.5 rounded">WINNER</span>}{ticket.status === 'ACTIVE' && <span className="text-xs font-bold text-blue-500 bg-blue-500/10 px-2 py-0.5 rounded">ACTIVE</span>}{ticket.status === 'LOST' && <span className="text-xs font-bold text-red-500 bg-red-500/10 px-2 py-0.5 rounded">LOST</span>}</div></div></div>
                           {ticket.status === 'ACTIVE' && (<button onClick={() => handleSwapTicket(ticket.id)} className="flex items-center gap-1 text-xs font-bold bg-yellow-500/10 text-yellow-600 px-3 py-2 rounded-lg hover:bg-yellow-500 hover:text-black transition-colors"><RefreshCw className="w-3 h-3"/> Swap (70%)</button>)}
                           {ticket.status === 'WON' && <Trophy className="w-6 h-6 text-yellow-500"/>}
                        </div>
                     ))}
                  </div>
               ) : (<div className={`p-12 border border-dashed rounded-3xl text-center text-neutral-500 ${isDark ? 'border-white/10' : 'border-black/10'}`}>You haven't entered any raffles yet.</div>)}
            </div>
          )}
        </div>
      </main>
      ...
      {ticketModalOpen && activeRaffle && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/60 backdrop-blur-md animate-in fade-in">
           <div className="bg-white w-full max-w-md p-8 rounded-[40px] relative shadow-2xl text-black">
              ...
              <div className="text-center mb-6"><div className="w-16 h-16 bg-black text-white rounded-full flex items-center justify-center mx-auto mb-4"><Ticket className="w-8 h-8"/></div><h3 className="text-2xl font-bold font-grotesk mb-2">Buy Raffle Ticket</h3><p className="text-neutral-500 text-sm">Enter the draw for this artifact.</p></div>
              <div className="bg-neutral-50 p-6 rounded-2xl mb-6 border border-neutral-200 text-center"><p className="text-xs text-neutral-500 uppercase tracking-widest mb-1">Ticket Cost</p><span className="text-3xl font-bold font-grotesk">{activeRaffle.price} ETH</span></div>
              <button onClick={handleBuyTicket} disabled={isPending || isSuccess} className="w-full py-4 bg-black text-white rounded-full font-bold text-lg hover:bg-neutral-800 transition-all disabled:opacity-50">{isPending ? 'Processing...' : isSuccess ? 'Ticket Purchased!' : 'Confirm Payment'}</button>
           </div>
        </div>
      )}
    </div>
